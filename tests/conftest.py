"""Test doubles that let a `AlfredPlayer` run without a Lavalink server."""
...
def set_position(player: AlfredPlayer, milliseconds: int) -> None:
    """
...
def confirm_playback(player: AlfredPlayer) -> None:
    """
...
@pytest.fixture
def player(node: FakeNode) -> AlfredPlayer:
    player = AlfredPlayer(guild_id=1, node=node)  # type: ignore[arg-type]
