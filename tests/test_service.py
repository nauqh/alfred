...
class FakeLavalinkClient:
    ...
    def __init__(self, player: AlfredPlayer | None = None, result: object = None) -> None:
...
@pytest.mark.asyncio
async def test_play_next_puts_the_track_at_the_front(player: AlfredPlayer) -> None:
    client = FakeLavalinkClient(player=player)
...
@pytest.mark.asyncio
async def test_looping_a_single_track_loops_just_that_track(player: AlfredPlayer) -> None:
    client = FakeLavalinkClient(player=player)

    await enqueue(client, lavalink.LoadResult.from_track(make_track()), loop=True)

    assert player.loop == AlfredPlayer.LOOP_SINGLE


@pytest.mark.asyncio
async def test_looping_a_playlist_loops_the_queue(player: AlfredPlayer) -> None:
    client = FakeLavalinkClient(player=player)

    await enqueue(client, playlist_result([make_track("a"), make_track("b")]), loop=True)

    assert player.loop == AlfredPlayer.LOOP_QUEUE


@pytest.mark.asyncio
async def test_an_unshuffled_playlist_keeps_its_order(player: AlfredPlayer) -> None:
    client = FakeLavalinkClient(player=player)
...
@pytest.mark.asyncio
async def test_playlist_tracks_remember_where_they_came_from(player: AlfredPlayer) -> None:
    client = FakeLavalinkClient(player=player)
...
@pytest.mark.asyncio
async def test_a_search_query_is_not_mistaken_for_a_playlist_link(player: AlfredPlayer) -> None:
    client = FakeLavalinkClient(player=player)
...
@pytest.mark.asyncio
async def test_queueing_nothing_raises_no_results(player: AlfredPlayer) -> None:
    client = FakeLavalinkClient(player=player)
